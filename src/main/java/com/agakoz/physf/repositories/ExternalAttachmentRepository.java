...
public interface ExternalAttachmentRepository extends JpaRepository<ExternalAttachment, Integer> {
    ...
    @Query("SELECT new com.agakoz.physf.model.DTO.ExternalAttachmentDTO(a.id, a.description, a.link, coalesce(a.file.id, -1)) from ExternalAttachment a where a.treatmentCycle.id = :treatmentCycleId")
    List<ExternalAttachmentDTO> findAttachmentsAssignedToTreatmentCycle(int treatmentCycleId);
